...
class MediaNotificationManager(private val mService: MediaPlaybackService) : BroadcastReceiver() {
    ...
    private fun createNotification(): Notification? {
        ...
        if (mPlaybackState!!.actions and PlaybackState.ACTION_SKIP_TO_PREVIOUS != 0L) {
            notificationBuilder.addAction(R.drawable.ic_skip_prev_black_24dp, mService.getString(R.string.skip_previous), mPreviousIntent)
        ...
        if (mPlaybackState!!.actions and PlaybackState.ACTION_SKIP_TO_NEXT != 0L) {
            notificationBuilder.addAction(R.drawable.ic_skip_next_black_24dp, mService.getString(R.string.skip_next), mNextIntent)
        }
        ...
        mMetadata?.description?.iconUri?.let {
            ...
            AlbumArtCache.getBigImage(artUrl)?.let { art = it } ?: let {
                ...
                art = BitmapFactory.decodeResource(mService.resources, R.drawable.albumart_mp_unknown)
            }
        }
        with(notificationBuilder) {
            ...
            color = ContextCompat.getColor(mService, R.color.primary)
            setSmallIcon(R.drawable.ic_podcasts_black_24dp)
    ...
    private fun addPlayPauseAction(builder: NotificationCompat.Builder) {
        ...
        val icon = if (playing) R.drawable.ic_pause_black_24dp else R.drawable.ic_play_arrow_black_24dp
