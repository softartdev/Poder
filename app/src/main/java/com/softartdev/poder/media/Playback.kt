 ...
 */
class Playback(private val mediaPlaybackService: MediaPlaybackService, private val mediaProvider: MediaProvider?) : AudioManager.OnAudioFocusChangeListener, OnCompletionListener, OnErrorListener, OnPreparedListener, OnSeekCompleteListener {
    private val mAudioManager: AudioManager = mediaPlaybackService.getSystemService(Context.AUDIO_SERVICE) as AudioManager
    // Create the Wifi lock (this does not acquire the lock, this just creates it)
    private val mWifiLock: WifiManager.WifiLock = (mediaPlaybackService.applicationContext.getSystemService(Context.WIFI_SERVICE) as WifiManager)
    ...
    private val mMediaPlayer: MediaPlayer by lazy {
        ...
        MediaPlayer().apply {
            ...
            setWakeMode(mediaPlaybackService.applicationContext, PowerManager.PARTIAL_WAKE_LOCK)
        ...
        override fun onReceive(context: Context, intent: Intent) {
            if (AudioManager.ACTION_AUDIO_BECOMING_NOISY == intent.action) {
                ...
                if (isPlaying) {
                    ...
                    mediaPlaybackService.startService(mediaIntent)
                }
            }
        }
    }
    ...
    private fun registerAudioNoisyReceiver() {
        if (!mAudioNoisyReceiverRegistered) {
            mediaPlaybackService.registerReceiver(mAudioNoisyReceiver, mAudioNoisyIntentFilter)
    ...
    private fun unregisterAudioNoisyReceiver() {
        if (mAudioNoisyReceiverRegistered) {
            mediaPlaybackService.unregisterReceiver(mAudioNoisyReceiver)
